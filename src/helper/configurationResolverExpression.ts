 ...
 * 
 * 実装メモ（日本語）:
 * - VSCode の configurationResolverExpression.ts を参考にしています。
 *
 * @template T The type of the root object (or string) being parsed and resolved.
 */
// 
...
export class ConfigurationResolverExpression<T> implements IConfigurationResolverExpression<T> {
	...
	public *unresolved(): Iterable<Replacement> {
		...
		while (true) {
            ...
            const next = newReplacements.entries().next().value;
			if (!next) {
				break;
			}
	...
	public resolved(): Iterable<[Replacement, IResolvedValue]> {
        ...
        const result: [Replacement, IResolvedValue][] = [];  
        for (const l of this.locations.values()) {
            if (l.resolved) {
                result.push([l.replacement, l.resolved]);
            }
        }
        return result;
	}
